...
void DAO::DataBase::_db_commit() {

}

void DAO::DataBase::_db_set_key(int p, int q, int key) {

}

void DAO::DataBase::db_enable() {
    db_enabled = 1;
}

void DAO::DataBase::db_disable() {
    db_enabled = 0;
}

int DAO::DataBase::get_db_enabled() {
    return db_enabled;
}

int DAO::DataBase::db_init(char *path) {
    if (!db_enabled) {
        return 0;
    }
...
void DAO::DataBase::db_commit() {
    if (!db_enabled) {
        return;
    }
    mtx_lock(&mtx);
    Entity::CppRing::ring_put_commit(&ring);
    cnd_signal(&cnd);
    mtx_unlock(&mtx);
}
...
void DAO::DataBase::db_auth_set(char *username, char *identity_token) {
    if (!db_enabled) {
        return;
    }
...
int DAO::DataBase::db_auth_select(char *username) {
    if (!db_enabled) {
        return 0;
    }
    db_auth_select_none();
...
void DAO::DataBase::db_auth_select_none() {
    if (!db_enabled) {
        return;
    }
...
int DAO::DataBase::db_auth_get_selected(
...
void DAO::DataBase::db_save_state(float x, float y, float z, float rx, float ry) {
    if (!db_enabled) {
        return;
    }
    static const char *query =
...
int DAO::DataBase::db_load_state(float *x, float *y, float *z, float *rx, float *ry) {
    if (!db_enabled) {
        return 0;
    }
    static const char *query =
...
void DAO::DataBase::db_insert_block(int p, int q, int x, int y, int z, int w) {
    if (!db_enabled) {
        return;
    }
    mtx_lock(&mtx);
    Entity::CppRing::ring_put_block(&ring, p, q, x, y, z, w);
    cnd_signal(&cnd);
    mtx_unlock(&mtx);
}
...
void DAO::DataBase::db_insert_light(int p, int q, int x, int y, int z, int w) {
    if (!db_enabled) {
        return;
    }
    mtx_lock(&mtx);
    Entity::CppRing::ring_put_light(&ring, p, q, x, y, z, w);
    cnd_signal(&cnd);
    mtx_unlock(&mtx);
}
...
void DAO::DataBase::db_insert_sign(
...
void DAO::DataBase::db_delete_sign(int x, int y, int z, int face) {
    if (!db_enabled) {
        return;
    }
...
void DAO::DataBase::db_delete_signs(int x, int y, int z) {
    if (!db_enabled) {
        return;
    }
...
void DAO::DataBase::db_delete_all_signs() {
    if (!db_enabled) {
        return;
    }
...
void DAO::DataBase::db_load_blocks(Map *map, int p, int q) {
    if (!db_enabled) {
        return;
    }
    mtx_lock(&load_mtx);
...
void DAO::DataBase::db_load_lights(Map *map, int p, int q) {
    if (!db_enabled) {
        return;
    }
    mtx_lock(&load_mtx);
...
void DAO::DataBase::db_load_signs(SignList *list, int p, int q) {
    if (!db_enabled) {
        return;
    }
    ...
    while (sqlite3_step(load_signs_stmt) == SQLITE_ROW) {
        ...
        const char *text = (const char *)sqlite3_column_text(
                load_signs_stmt, 4);
        DataStruct::CppSign::sign_list_add(list, x, y, z, face, text);
    }
}

int DAO::DataBase::db_get_key(int p, int q) {
    if (!db_enabled) {
        return 0;
    }
...
void DAO::DataBase::db_set_key(int p, int q, int key) {
    if (!db_enabled) {
        return;
    }
    mtx_lock(&mtx);
    Entity::CppRing::ring_put_key(&ring, p, q, key);
    cnd_signal(&cnd);
    mtx_unlock(&mtx);
}
...
void DAO::DataBase::db_worker_start() {

}


void DAO::DataBase::db_worker_start(char *path) {
    if (!db_enabled) {
        return;
    }
    Entity::CppRing::ring_alloc(&ring, 1024);
...
void DAO::DataBase::db_worker_stop() {
    if (!db_enabled) {
        return;
    }
    mtx_lock(&mtx);
    Entity::CppRing::ring_put_exit(&ring);
    cnd_signal(&cnd);
    mtx_unlock(&mtx);
    thrd_join(thrd, NULL);
    cnd_destroy(&cnd);
    ...
    mtx_destroy(&mtx);
    Entity::CppRing::ring_free(&ring);
}

int DAO::DataBase::db_worker_run(void *arg) {
    int running = 1;
    while (running) {
        RingEntry e;
        mtx_lock(&mtx);
        while (!Entity::CppRing::ring_get(&ring, &e)) {
            cnd_wait(&cnd, &mtx);
        }
        mtx_unlock(&mtx);
        switch (e.type) {
