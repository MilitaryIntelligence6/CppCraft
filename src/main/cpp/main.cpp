...
GLuint gen_crosshair_buffer() {
    ...
    return Util::Utility::gen_buffer(sizeof(data), data);
}

GLuint gen_wireframe_buffer(float x, float y, float z, float n) {
    float data[72];
    Entity::Cube::make_cube_wireframe(data, x, y, z, n);
    return Util::Utility::gen_buffer(sizeof(data), data);
}

GLuint gen_sky_buffer() {
    float data[12288];
    Entity::Cube::make_sphere(data, 1, 3);
    return Util::Utility::gen_buffer(sizeof(data), data);
}

GLuint gen_cube_buffer(float x, float y, float z, float n, int w) {
    GLfloat *data = Util::Utility::malloc_faces(10, 6);
    float ao[6][4] = {0};
    float light[6][4] = {
            {0.5, 0.5, 0.5, 0.5},
            {0.5, 0.5, 0.5, 0.5},
    ...
    Entity::Cube::make_cube(data, ao, light, 1, 1, 1, 1, 1, 1, x, y, z, n, w);
    return Util::Utility::gen_faces(10, 6, data);
}

GLuint gen_plant_buffer(float x, float y, float z, float n, int w) {
    GLfloat *data = Util::Utility::malloc_faces(10, 4);
    float ao = 0;
    float light = 1;
    Entity::Cube::make_plant(data, ao, light, x, y, z, n, w, 45);
    return Util::Utility::gen_faces(10, 4, data);
}

GLuint gen_player_buffer(float x, float y, float z, float rx, float ry) {
    GLfloat *data = Util::Utility::malloc_faces(10, 6);
    Entity::Cube::make_player(data, x, y, z, rx, ry);
    return Util::Utility::gen_faces(10, 6, data);
}

GLuint gen_text_buffer(float x, float y, float n, char *text) {
    int length = strlen(text);
    GLfloat *data = Util::Utility::malloc_faces(4, length);
    for (int i = 0; i < length; i++) {
        Entity::Cube::make_character(data + i * 24, x, y, n / 2, n, text[i]);
        x += n;
    }
    return Util::Utility::gen_faces(4, length, data);
}
...
int collide(int height, float *x, float *y, float *z) {
    ...
    for (int dy = 0; dy < height; dy++) {
        if (px < -pad && Entity::Item::is_obstacle(Scene::WorldMap::map_get(map, nx - 1, ny - dy, nz))) {
            *x = nx - pad;
        }
        if (px > pad && Entity::Item::is_obstacle(Scene::WorldMap::map_get(map, nx + 1, ny - dy, nz))) {
            *x = nx + pad;
        }
        if (py < -pad && Entity::Item::is_obstacle(Scene::WorldMap::map_get(map, nx, ny - dy - 1, nz))) {
            *y = ny - pad;
            result = 1;
        }
        if (py > pad && Entity::Item::is_obstacle(Scene::WorldMap::map_get(map, nx, ny - dy + 1, nz))) {
        ...
        if (pz < -pad && Entity::Item::is_obstacle(Scene::WorldMap::map_get(map, nx, ny - dy, nz - 1))) {
            *z = nz - pad;
        }
        if (pz > pad && Entity::Item::is_obstacle(Scene::WorldMap::map_get(map, nx, ny - dy, nz + 1))) {
            *z = nz + pad;
...
int _gen_sign_buffer(
    ...
    char lines[1024];
    int rows = Util::Utility::wrap(text, max_width, lines, 1024);
    rows = MIN(rows, 5);
    ...
    char *key;
    char *line = Util::Utility::tokenize(lines, "\n", &key);
    while (line) {
        int length = strlen(line);
        int line_width = Util::Utility::string_width(line);
        line_width = MIN(line_width, max_width);
        float rx = sx - dx * line_width / max_width / 2;
        float ry = sy;
        float rz = sz - dz * line_width / max_width / 2;
        for (int i = 0; i < length; i++) {
            int width = Util::Utility::char_width(line[i]);
            line_width -= width;
            if (line_width < 0) {
                break;
            }
...
void gen_sign_buffer(Chunk *chunk) {
    ...
    GLfloat *data = Util::Utility::malloc_faces(5, max_faces);
    int faces = 0;
    for (int i = 0; i < signs->size; i++) {
        Sign *e = signs->data + i;
    ...
    Util::Utility::del_buffer(chunk->sign_buffer);
    chunk->sign_buffer = Util::Utility::gen_faces(5, faces, data);
...
void generate_chunk(Chunk *chunk, WorkerItem *item) {
    ...
    Util::Utility::del_buffer(chunk->buffer);
    chunk->buffer = Util::Utility::gen_faces(10, item->faces, item->data);
...
void load_chunk(WorkerItem *item) {
    ...
    new Scene::World(p, q, map_set_func, block_map);
    DAO::DataBase::db_load_blocks(block_map, p, q);
    DAO::DataBase::db_load_lights(light_map, p, q);
}

void request_chunk(int p, int q) {
    int key = DAO::DataBase::db_get_key(p, q);
    Controller::Client::client_chunk(p, q, key);
}
...
void init_chunk(Chunk *chunk, int p, int q) {
    ...
    dirty_chunk(chunk);
    SignList *signs = &chunk->signs;
    DataStruct::CppSign::sign_list_alloc(signs, 16);
    DAO::DataBase::db_load_signs(signs, p, q);
    Map *block_map = &chunk->map;
    Map *light_map = &chunk->lights;
    int dx = p * CHUNK_SIZE - 1;
    int dy = 0;
    int dz = q * CHUNK_SIZE - 1;
    Scene::WorldMap::map_alloc(block_map, dx, dy, dz, 0x7fff);
    Scene::WorldMap::map_alloc(light_map, dx, dy, dz, 0xf);
}
...
void delete_chunks() {
    ...
    for (int i = 0; i < count; i++) {
        ...
        if (the_delete) {
            Scene::WorldMap::map_free(&chunk->map);
            Scene::WorldMap::map_free(&chunk->lights);
            DataStruct::CppSign::sign_list_free(&chunk->signs);
            Util::Utility::del_buffer(chunk->buffer);
            Util::Utility::del_buffer(chunk->sign_buffer);
        ...
        if (worker->state == WORKER_DONE) {
            ...
            if (chunk) {
                if (item->load) {
                    ...
                    Scene::WorldMap::map_free(&chunk->map);
                    Scene::WorldMap::map_free(&chunk->lights);
                    Scene::WorldMap::map_copy(&chunk->map, block_map);
                    Scene::WorldMap::map_copy(&chunk->lights, light_map);
...
void ensure_chunks_worker(Player *player, Worker *worker) {
    State *s = &player->state;
    float matrix[16];
    Graphic::Matrix::set_matrix_3d(
            matrix, g->width, g->height,
            s->x, s->y, s->z, s->rx, s->ry, g->fov, g->ortho, g->render_radius);
    float planes[6][4];
    Graphic::Matrix::frustum_planes(planes, g->render_radius, matrix);
    int p = chunked(s->x);
    int q = chunked(s->z);
...
void unset_sign(int x, int y, int z) {
    ...
    if (chunk) {
        SignList *signs = &chunk->signs;
        if (DataStruct::CppSign::sign_list_remove_all(signs, x, y, z)) {
            chunk->dirty = 1;
...
void unset_sign_face(int x, int y, int z, int face) {
    ...
    if (chunk) {
        SignList *signs = &chunk->signs;
        if (DataStruct::CppSign::sign_list_remove(signs, x, y, z, face)) {
            chunk->dirty = 1;
...
void _set_sign(
    ...
    if (chunk) {
        SignList *signs = &chunk->signs;
        DataStruct::CppSign::sign_list_add(signs, x, y, z, face, text);
        if (dirty) {
            chunk->dirty = 1;
        }
    }
    DAO::DataBase::db_insert_sign(p, q, x, y, z, face, text);
}
...
void set_sign(int x, int y, int z, int face, const char *text) {
    ...
    Controller::Client::client_sign(x, y, z, face, text);
}
...
void toggle_light(int x, int y, int z) {
    ...
    if (chunk) {
        ...
        int w = Scene::WorldMap::map_get(map, x, y, z) ? 0 : 15;
        Scene::WorldMap::map_set(map, x, y, z, w);
        DAO::DataBase::db_insert_light(p, q, x, y, z, w);
        Controller::Client::client_light(x, y, z, w);
        dirty_chunk(chunk);
    }
}
...
void set_light(int p, int q, int x, int y, int z, int w) {
    ...
    if (chunk) {
        Map *map = &chunk->lights;
        if (Scene::WorldMap::map_set(map, x, y, z, w)) {
            dirty_chunk(chunk);
            DAO::DataBase::db_insert_light(p, q, x, y, z, w);
        }
    } else {
...
void set_block(int x, int y, int z, int w) {
    ...
    Controller::Client::client_block(x, y, z, w);
}
...
int get_block(int x, int y, int z) {
    ...
    if (chunk) {
        Map *map = &chunk->map;
        return Scene::WorldMap::map_get(map, x, y, z);
    }
    return 0;
}
...
void builder_block(int x, int y, int z, int w) {
    ...
    if (Entity::Item::is_destructable(get_block(x, y, z))) {
        set_block(x, y, z, 0);
    }
    if (w) {
        set_block(x, y, z, w);
...
void render_sign(Attrib *attrib, Player *player) {
    ...
    GLfloat *data = Util::Utility::malloc_faces(5, strlen(text));
    int length = _gen_sign_buffer(data, x, y, z, face, text);
    GLuint buffer = Util::Utility::gen_faces(5, length, data);
...
void render_wireframe(Attrib *attrib, Player *player) {
    ...
    if (Entity::Item::is_obstacle(hw)) {
        ...
        Util::Utility::del_buffer(wireframe_buffer);
...
void render_crosshairs(Attrib *attrib) {
    ...
    Util::Utility::del_buffer(crosshair_buffer);
    glDisable(GL_COLOR_LOGIC_OP);
}

void render_item(Attrib *attrib) {
    float matrix[16];
    Graphic::Matrix::set_matrix_item(matrix, g->width, g->height, g->scale);
    glUseProgram(attrib->program);
    glUniformMatrix4fv(attrib->matrix, 1, GL_FALSE, matrix);
...
void on_light() {
    ...
    int hx, hy, hz;
    int hw = hit_test(0, s->x, s->y, s->z, s->rx, s->ry, &hx, &hy, &hz);
    if (hy > 0 && hy < 256 && Entity::Item::is_destructable(hw)) {
...
void on_left_click() {
    ...
    if (hy > 0 && hy < 256 && Entity::Item::is_destructable(hw)) {
        ...
        if (Entity::Item::is_plant(get_block(hx, hy + 1, hz))) {
...
void on_right_click() {
    ...
    if (hy > 0 && hy < 256 && Entity::Item::is_obstacle(hw)) {
...
void parse_buffer(char *buffer) {
    ...
    State *s = &g->players->state;
    char *key;
    char *line = Util::Utility::tokenize(buffer, "\n", &key);
    while (line) {
        int pid;
        ...
        if (sscanf(line, "K,%d,%d,%d", &kp, &kq, &kk) == 3) {
            DAO::DataBase::db_set_key(kp, kq, kk);
        }
...
int main(int argc, char **argv) {
    ...
    Util::Utility::load_png_texture("textures/texture.png");
    ...
    Util::Utility::load_png_texture("textures/font.png");
    ...
    Util::Utility::load_png_texture("textures/sky.png");
    ...
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
    Util::Utility::load_png_texture("textures/sign.png");
    ...
    program = Util::Utility::load_program(
    ...
    while (running) {
        ...
        if (g->mode == MODE_OFFLINE || USE_CACHE) {
            DAO::DataBase::db_enable();
            if (DAO::DataBase::db_init(g->db_path)) {
                return -1;
            }
            if (g->mode == MODE_ONLINE) {
                // TODO: support proper caching of signs (handle deletions)
                DAO::DataBase::db_delete_all_signs();
            }
        }

        // CLIENT INITIALIZATION //
        if (g->mode == MODE_ONLINE) {
            Controller::Client::client_enable();
            Controller::Client::client_connect(g->server_addr, g->server_port);
            Controller::Client::client_start();
            Controller::Client::client_version(1);
            login();
        }
        ...
        int loaded = DAO::DataBase::db_load_state(&s->x, &s->y, &s->z, &s->rx, &s->ry);
        force_chunks(me);
        if (!loaded) {
            s->y = highest_block(s->x, s->z) + 2;
        }
...
//                free(buffer);
            ...
            if (now - last_commit > COMMIT_INTERVAL) {
                last_commit = now;
                DAO::DataBase::db_commit();
            }

            // SEND POSITION TO SERVER //
            if (now - last_update > 0.1) {
                last_update = now;
                Controller::Client::client_position(s->x, s->y, s->z, s->rx, s->ry);
            }
            ...
            delete_chunks();
            Util::Utility::del_buffer(me->buffer);
        ...
        // SHUTDOWN //
        DAO::DataBase::db_save_state(s->x, s->y, s->z, s->rx, s->ry);
        DAO::DataBase::db_close();
        DAO::DataBase::db_disable();
        Controller::Client::client_stop();
        Controller::Client::client_disable();
        Util::Utility::del_buffer(sky_buffer);
        delete_all_chunks();
        delete_all_players();
    }
